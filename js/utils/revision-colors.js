...
export const revisionAreaColorMethods = {
    ...
    getRevisionAreaColor(sectionId) {
        ...
        // Debug first few unique sections
        if (!window._debuggedSections) window._debuggedSections = new Set();
        if (window._debuggedSections.size < 15 && !window._debuggedSections.has(sectionId)) {
            console.log(`[Section Color] "${sectionId}" → Hash:${hash} → Hue:${hue} → ${hex}`);
            window._debuggedSections.add(sectionId);
        }
    ...
    getColorForTopic(topicId) {
        ...
        // Debug logging using a global counter
        if (!window._colorDebugCount) window._colorDebugCount = 0;
        if (window._colorDebugCount < 20) {
            console.log(`[Color ${window._colorDebugCount}] Topic "${topicId}" → Section "${sectionId}" → Hue ${color.hex}`);

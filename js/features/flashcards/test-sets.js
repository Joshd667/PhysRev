...
export const testSetMethods = {
    ...
    saveTestSet() {
        ...
        if (this.testSetBuilderCards.length === 0) {
            alert('Please add at least one card to the test set');
            return;
        }
    ...
    deleteTestSet(testSetId) {
        ...
        if (confirm(`Delete test set "${testSet.name}"?`)) {
     ...
     */
    startTestFromSet(testSetId) {
        const testSet = this.testSets[testSetId];
        if (!testSet) return;

        if (testSet.cards.length === 0) {
            alert('This test set has no cards');
            return;
        }
    ...
    startQuickPlayTest() {
        ...
        if (allCards.length === 0) {
            alert('No flashcards available for quick play');
            return;
        }

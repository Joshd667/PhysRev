...
export const tagManagementMethods = {
    ...
    addTag(topicId) {
        ...
        switch (this.tagSelectorContext) {
            ...
            case 'search':
                ...
                this.performSearch(); // Trigger search when tag is added
                break;
        }
    },
    ...
    removeTag(topicId) {
        switch (this.tagSelectorContext) {
            ...
            case 'search':
                ...
                this.performSearch(); // Trigger search when tag is removed
                break;
        }
    },
    ...
    /**
     * Checks if all topics in a section are selected
     * @param {Object} section - The section object with topics array
     * @returns {boolean} True if all topics in section are selected
     */
    areAllTopicsInSectionSelected(section) {
        if (!section.topics || section.topics.length === 0) {
            return false;
        }
        const currentTags = this.getCurrentTags();
        return section.topics.every(topic => currentTags.includes(topic.topicId));
    },

    /**
     * Toggles selection of all topics in a section
     * @param {Object} section - The section object with topics array
     */
    toggleAllTopicsInSection(section) {
        if (!section.topics || section.topics.length === 0) {
            return;
        }

        const currentTags = this.getCurrentTags();
        const allSelected = section.topics.every(topic => currentTags.includes(topic.topicId));

        if (allSelected) {
            // Deselect all topics in this section
            section.topics.forEach(topic => {
                const index = currentTags.indexOf(topic.topicId);
                if (index > -1) {
                    switch (this.tagSelectorContext) {
                        case 'note':
                            this.noteEditorTags = this.noteEditorTags.filter(t => t !== topic.topicId);
                            break;
                        case 'flashcard':
                            this.flashcardEditorTags = this.flashcardEditorTags.filter(t => t !== topic.topicId);
                            break;
                        case 'mindmap':
                            this.mindmapEditorTags = this.mindmapEditorTags.filter(t => t !== topic.topicId);
                            break;
                        case 'advancedSearch':
                            this.advancedSearchTags = this.advancedSearchTags.filter(t => t !== topic.topicId);
                            break;
                        case 'search':
                            this.selectedSearchTags = this.selectedSearchTags.filter(t => t !== topic.topicId);
                            break;
                    }
                }
            });
            ...
            section.topics.forEach(topic => {
                if (!currentTags.includes(topic.topicId)) {
                    switch (this.tagSelectorContext) {
                        case 'note':
                            this.noteEditorTags.push(topic.topicId);
                            break;
                        case 'flashcard':
                            this.flashcardEditorTags.push(topic.topicId);
                            break;
                        case 'mindmap':
                            this.mindmapEditorTags.push(topic.topicId);
                            break;
                        case 'advancedSearch':
                            this.advancedSearchTags.push(topic.topicId);
                            break;
                        case 'search':
                            this.selectedSearchTags.push(topic.topicId);
                            break;
                    }
                }
            });
        ...
        // Trigger search only once if in search context
        if (this.tagSelectorContext === 'search') {
            this.performSearch();
            this.$nextTick(() => {
                if (window.lucide) {
                    lucide.createIcons();
